...
def _filter(regexp: str) -> tuple[str, ...]:
    import re
    r = re.compile(regexp)
    return tuple(e for e in all if r.fullmatch(e))


timeperiods = _filter(r'(?:late |early )?\d{4}s|eighties')
years = _filter(r'\d{4}')
jazz = _filter(r'.*jazz.*|.*fusion.*|.*bop|dixieland')
swing = _filter(r'.*swing.*')
soul = _filter(r'.*soul.*')
funk = _filter(r'.*funk.*')
house = _filter(r'.*house.*|future (?:funk|rave)|footwork.*|.* bounce|dream trance|electro swing|dembow|amapiano|complextro|future funck|hardbass|.* beat|.* club|(?:uk|.*future.*|miami|color|brazilian|atlanta|global|) bass')
techno = _filter(r'.*techno.*|schranz|tech trance|tech house')
trance = _filter(r'.*trance.*')
chipmusic = _filter(r'suomisaundi|fm synthesis|game music|chip music|chip|chiptune|nintendocore|gameboy|bitcore|bit')
vapor = _filter(r'.*vapou?r.*')

# sjekk: aggrotech, ghettotech, techstep, techno kayō, techno bass

classical_old = _filter(r'.*baroque.*|.*renaissance.*')

norwegian = _filter(r'.*norsk.*|.*norwegian.*|hardingfele')
swedish = _filter(r'.*svensk.*|.*swedish.*|dansbandsmusik|dansband')
danish = _filter(r'.*dansk.*|.*danish.*')
finland = _filter(r'.*suomi.*|.*finland.*|.*finnish.*')
scandinavian = _filter('.*scandi.*')
nordic = _filter('.*nordic.*')
arabic = _filter(r'.*arabic.*')

german = _filter(r'.*deutsch.*|.*german.*')
dutch = _filter(r'.*neder.*|.*dutch.*')


electronic_psy: tuple[str, ...] = (
    'psytrance',
    'psybient',
    'psychill',
    'full-on psytrance',
